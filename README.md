...
- 박설아: 알고리즘에 대한 개념이 잡혀있지 않은 상태에서 처음으로 참여한 데이터 공모전이었습니다. 데이터를 이해하는 능력과 데이터에 적합한 머신러닝 방식을 찾아보는 것이 중요하다고 느꼈습니다. 공모전 활동을 이해하기에 조금 오랜 시간이 걸렸지만, 같이 참여한 팀원의 리더십으로 첫 공모전 활동에서 꽤 만족스러운 결과를 얻을 수 있던 것 같았고, 데이터 활용에 있어서 더 많은 공부를 해야 할 것 같다고 느꼈습니다.
